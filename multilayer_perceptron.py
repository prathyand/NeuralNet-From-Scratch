...
#
# Submitted by: [Prathmesh Deshmukh] -- [pdeshmuk]
#
...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        self.n_output=self._y.shape[1]
        np.random.seed(42)
        self._h_weights = -1+2*np.random.rand(self.n_features,self.n_hidden)
        self._h_bias = -1+2*np.random.rand(1,self.n_hidden)
        self._o_weights = -1+2*np.random.rand(self.n_hidden,self.n_output)
        self._o_bias = -1+2*np.random.rand(1,self.n_output)
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_iterations):
            #feed forward step
            self.z1=np.matmul(self._X,self._h_weights)+self._h_bias
            self.a1=self.hidden_activation(self.z1)
            self.z2=np.matmul(self.a1,self._o_weights)+self._o_bias
            self.ypred=self._output_activation(self.z2)

            #error and derivatives calculations and weight update for output layer
            er=self.ypred-self._y
            del_o=er*self._output_activation(self.z2,derivative=True)
            deriv_o=np.matmul(self.a1.T,del_o)

            self._o_weights=self._o_weights-self.learning_rate*deriv_o
            self._o_bias=self._o_bias-self.learning_rate*np.sum(del_o,axis=0)

            #error and derivatives calculations and weight update for hidden layer
            del_h=np.matmul(del_o,self._o_weights.T)*self.hidden_activation(self.z1,derivative=True)
            deriv_h=np.matmul(self._X.T,del_h)

            self._h_weights=self._h_weights-self.learning_rate*deriv_h
            self._h_bias=self._h_bias-self.learning_rate*np.sum(deriv_h,axis=0)
    ...
    def predict(self, X):
        ...
        """
        outt=self._output_activation(np.matmul(self.hidden_activation(np.matmul(X,self._h_weights)+self._h_bias),self._o_weights)+self._o_bias)
        return np.array([self.classlist[ind] for ind in np.argmax(outt,axis=1)])
